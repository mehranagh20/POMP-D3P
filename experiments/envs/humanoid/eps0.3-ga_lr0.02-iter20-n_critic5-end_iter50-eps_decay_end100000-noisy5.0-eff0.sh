...
# eps0.3-ga_lr0.02-iter20-n_critic5-end_iter50-eps_decay_end100000-noisy5.0-eff0
...
for seed in 1 2 3 4 5 7; do
    name=eps${eps}-ga_lr${ga_lr}-iter${iter}-n_critic${n_critic}-end_iter${end_iter}-eps_decay_end${eps_decay_end}-noisy${noisy_coef}-seed${seed}-long
    setting=eps${eps}-ga_lr${ga_lr}-iter${iter}-n_critic${n_critic}-end_iter${end_iter}-eps_decay_end${eps_decay_end}-noisy${noisy_coef}
    project_name=humanoid-best
